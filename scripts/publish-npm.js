...
const buildAndroid = rebuildOnError => {
  ...
  const {stdout: aarContent} = exec(
    `unzip -l ${aarLocation} | grep libfbjni.so`,
  );
  ...
  if (rebuildOnError) {
    echo(
      `${aarLocation} is missing contents. Rebuilding with Gradle again in an effort to try to fix it.`,
    );
    buildAndroid(false);
  } else {
    echo(
      `${aarLocation} is missing contents. Rebuilding did not fix the issue.`,
    );
    exit(1);
  }
};
